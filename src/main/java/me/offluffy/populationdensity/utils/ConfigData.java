...
public class ConfigData {
    ...
    public ConfigData() {
        ...
        List<String> defaultManagedWorldNames = new ArrayList<>();

        //build a list of normal environment worlds
        List<World> worlds = Bukkit.getWorlds();
        ArrayList<World> normalWorlds = new ArrayList<>();
        for (World world : worlds) if (world.getEnvironment() == Environment.NORMAL) normalWorlds.add(world);
    ...
    private String[] initializeSignContentConfig(String node, String... defaultLines) {
        ...
        if (nodeData == null || nodeData.size() == 0)
            for (; i < defaultLines.length && i < 4; i++) nodeData.add(defaultLines[i]);

        for (i = nodeData.size(); i < 4; i++) nodeData.add("");
