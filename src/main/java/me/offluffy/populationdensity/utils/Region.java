...
public class Region {
    ...
    private static void scanRegion(Region region, boolean openNewRegions) {
        ...
        do {
            ...
            for (int x = 0; x < snapshots.length; x++) {
                for (int z = 0; z < snapshots[0].length; z++) {
                    ...
                    while (!chunk.load(true)) if (chunk.isLoaded()) break;
        ...
        for (Chunk loadedChunk : loadedChunks) loadedChunk.unload(true, true);  //save = true, safe = true
        //collect garbage
        System.gc(); // TODO: Examine if this should be called (probably not)
    ...
    public static Region getRandomRegion(Region regionToAvoid) {
        if (Region.regionCount < 2) return null;
        ...
        for (File file : files) {
            if (file.isFile()) { //avoid any folders
                try {
                    ...
                    Region regionCoordinates = new Region(file.getName());
                    if (!regionCoordinates.equals(regionToAvoid)) regions.add(regionCoordinates);
    ...
    public void addPost(boolean updateNeighboringRegions) {
        ...
        if (!ConfigData.buildNamedPosts && !ConfigData.buildWildPosts) return;
        ...
        if (y < ConfigData.minimumRegionPostY) y = ConfigData.minimumRegionPostY;
        ...
        if (regionName != null) {
            ...
            Sign signData = new Sign(Material.WALL_SIGN);
        ...
        if (ConfigData.cityWorld != null) {
            ...
            signData = new Sign(Material.WALL_SIGN);
            signData.setFacingDirection(BlockFace.NORTH);
            block.setTypeIdAndData(Material.WALL_SIGN.getId(), signData.getData(), false);
        ...
        HashMap<BlockFace, String[]> content = new HashMap<>();
