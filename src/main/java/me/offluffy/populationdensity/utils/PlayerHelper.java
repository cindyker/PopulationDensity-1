...
public class PlayerHelper {
    ...
    public static void removeData(String player) {
        if (!nameDataMap.containsKey(player)) return;
    ...
    public static boolean playerCanTeleport(Player player, boolean isHomeOrCityTeleport) {
        ...
        if (Lib.perm(player, "populationdensity.teleportanywhere")) return true;
        //if teleportation from anywhere is enabled, always allow it
        if (ConfigData.teleportFromAnywhere) return true;
    ...
    public static boolean nearCityPost(Player player) {
        return ConfigData.cityWorld != null && player.getWorld().equals(ConfigData.cityWorld) && (ConfigData.postTeleportRadius < 1 || player.getLocation().distance(ConfigData.cityWorld.getSpawnLocation()) < ConfigData.postTeleportRadius);
    }
    ...
    public static void teleportPlayer(Player player, Region region, boolean silent) {
        ...
        if (Region.getRegionName(region) != null) regName = Clr.HEAD + Lib.capitalize(Region.getRegionName(region));
        //notify him
        if (!silent) player.sendMessage(Clr.NORM + "Teleporting you to " + regName);
        //send him
    ...
    private static void loadPlayerData(String playerName) {
        ...
        try {
            ...
            playerData.homeRegion = new Region(homeCoords);
            ...
            try {
                ...
                playerData.lastDisconnect = dateFormat.parse(lastDisconnectedString);
            } catch (Exception e) {
                playerData.lastDisconnect = Calendar.getInstance().getTime();
            }
            //parse priority string
            if (rankString == null || rankString.isEmpty()) playerData.loginPriority = 0;
            else {
                try {
