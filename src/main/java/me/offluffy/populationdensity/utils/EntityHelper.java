...
public class EntityHelper {
    ...
    public static List<Entity> getTypeInChunk(Chunk chunk, EntityGroup group) {
        ...
        for (Entity ent : chunk.getEntities()) if (entEq(ent.getType(), getGroups(group))) ents.add(ent);
        return ents;
    }
    ...
    public static EntityType getType(String type) {
        if (EntityType.fromName(type) != null) return EntityType.fromName(type);
            ...
            for (EntityType t : EntityType.values()) {
                if (type.equalsIgnoreCase(t.getName())) return t;
                else if (type.equalsIgnoreCase(t.toString())) return t;
                else if (type.equalsIgnoreCase(t.toString().replace("_", ""))) return t;
            }
        }
    ...
    private static boolean entEq(EntityType ent, EntityType... types) {
        for (EntityType type : types) if (ent.equals(type)) return true;
        return false;
    }
    ...
    public static List<LivingEntity> getTargets(Location l, List<EntityType> types, long rad) {
        ...
        for (Entity e : l.getWorld().getEntities()) {
            if (e instanceof LivingEntity) {
                ...
                if (!le.contains(lEnt)) for (EntityType type : types) if (lEnt.getType().equals(type)) le.add(lEnt);
            }
        }
        Block b = l.getBlock();
