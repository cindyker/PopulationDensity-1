			...
			for(int i = 0; i < chunks.length; i++) {
				...
				for(int j = 0; j < entities.length; j++) {
					...
					if(EntityHelper.isAnimal(entity.getType())) {
						...
						cmc++; gmc++;
						if(cmc > ConfigData.maxMonsters) { entity.remove(); gmr++; }
					} else if(EntityHelper.isVillager(entity.getType())) {
						cvc++; gvc++;
						if(cvc > ConfigData.maxVillagers) { entity.remove(); gvr++; }
					} else if(EntityHelper.isGolem(entity.getType())) {
						cgc++; ggc++;
						if (cgc > ConfigData.maxGolems) { entity.remove(); ggr++; }
					} else if(EntityHelper.isDrop(entity.getType())) {
						cdc++; gdc++;
						if(cdc > ConfigData.maxDrops) { entity.remove(); gdr++; }
					}
				}
